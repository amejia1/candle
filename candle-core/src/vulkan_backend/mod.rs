...
// impl BackendStorage for VulkanStorageOld {
...
//                     Ok(self.clone())
//                 } else {
//                     crate::bail!("Vulkan to_dtype {left:?} {right:?} not implemented")
//                 }
//             }
//         }
//     }
...
// #[derive(Clone)]
// pub struct VulkanStorageOld {
//     pub(crate) buffer: VBuf,
//     pub(crate) device: VulkanDevice,
//     pub(crate) data_len: usize,
//     pub(crate) dtype: DType,
// }

// impl std::fmt::Debug for VulkanStorageOld {
...
//         let _ = crate::backend::BackendDevice::storage_from_cpu_storage(device, &cpu)?;
//         // Ok(storage)
//         todo!()
//     }
// }
...
impl BackendDevice for VulkanDevice {
    ...
    fn zeros_impl(&self, shape: &Shape, dtype: DType) -> Result<Self::Storage> {
        VulkanStorage::new(self, shape, dtype)
    }
...
impl VulkanStorage {
    pub fn new(device: &VulkanDevice, shape: &Shape, dtype: DType) -> Result<Self> {
        let size_in_bytes = dtype.size_in_bytes();
        let size = if size_in_bytes >= 1 {
            shape.elem_count() * size_in_bytes
        } else {
            // Some dtypes are marked as having zero bytes. Treat these as needing at least 1 byte.
            shape.elem_count()
        };
        let slice = match dtype {
            DType::U8 => VulkanStorageSlice::U8(Self::create_vram_buffer::<u8>(device, size)?),
            DType::U32 => VulkanStorageSlice::U32(Self::create_vram_buffer::<u32>(device, size)?),
            DType::I16 => VulkanStorageSlice::I16(Self::create_vram_buffer::<i16>(device, size)?),
            DType::I32 => VulkanStorageSlice::I32(Self::create_vram_buffer::<i32>(device, size)?),
            DType::I64 => VulkanStorageSlice::I64(Self::create_vram_buffer::<i64>(device, size)?),
            DType::BF16 => {
                VulkanStorageSlice::BF16(Self::create_vram_buffer::<half::bf16>(device, size)?)
            }
            DType::F16 => {
                VulkanStorageSlice::F16(Self::create_vram_buffer::<half::f16>(device, size)?)
            }
            DType::F32 => VulkanStorageSlice::F32(Self::create_vram_buffer::<f32>(device, size)?),
            DType::F64 => VulkanStorageSlice::F64(Self::create_vram_buffer::<f64>(device, size)?),
            DType::F8E4M3 => {
                VulkanStorageSlice::F8E4M3(Self::create_vram_buffer::<u8>(device, size)?)
            }
            DType::F6E2M3 => VulkanStorageSlice::U8(Self::create_vram_buffer::<u8>(device, size)?),
            DType::F6E3M2 => VulkanStorageSlice::U8(Self::create_vram_buffer::<u8>(device, size)?),
            DType::F4 => VulkanStorageSlice::U8(Self::create_vram_buffer::<u8>(device, size)?),
            DType::F8E8M0 => VulkanStorageSlice::U8(Self::create_vram_buffer::<u8>(device, size)?),
        };
        Ok(Self { slice })
    }
